...
nome_contador = "TERAHOST-PLAY"
link_contador = "https://whos.amung.us/pingjs/"    


##CONFIGURAÇÕES
####  TITULO DO MENU  #################################################################
title_menu = "[B][COLOR white]||||||| TERAHOST-PLAY|||||||[/COLOR][/B]"
###  DESCRIÇÃO DO ADDON ###############################################################
title_descricao = "DESCRIÇÃO ADDON"
...
def init_SKindex(msg):
   ...
   if status_mensagem1 == 'true':
    dialog = xbmcgui.Dialog()
    link = dialog.select("[B]BEM-VINDOS AO [COLOR white]TERAHOST-PLAY[/COLOR][/B]", [
    "[B][COLOR white]EXEMPLO - [/COLOR] FACEBOOK [/B]",
    "[B][COLOR white]EXEMPLO - [/COLOR] INSTAGRAM [/B]",
    "[B][COLOR white]EXEMPLO - [/COLOR] TWITTER [/B]",
    "[B][COLOR white]EXEMPLO - [/COLOR] ENTRAR NO ADD-ON [/B]"])    
      
    if link == 0:
     if xbmc . getCondVisibility ( 'system.platform.android' ) :
         xbmc . executebuiltin ( 'StartAndroidActivity(,android.intent.action.VIEW,,%s)' % ( '#' ) )
     else:
        ost = webbrowser . open ( '#' )
    ...
    if link == 2:
     if xbmc . getCondVisibility ( 'system.platform.android' ) :
         ost = xbmc . executebuiltin ( 'StartAndroidActivity(,android.intent.action.VIEW,,%s)' % ( '#' ) )
     else:
        webbrowser . open ( '#' ) 
